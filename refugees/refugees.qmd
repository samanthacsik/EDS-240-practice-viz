...
<https://github.com/cran/refugees>
...
## line plot
```{r}
ggplot(pop_clean, aes(x = date, y = total_refugees, group = origin)) +
  geom_line() 
...
## highlight lines
```{r}
max_countries <- pop_clean |> 
  group_by(origin) |> 
  summarize(
    max_num_ref = max(total_refugees)
  ) |> 
  arrange(desc(max_num_ref))

ggplot(pop_clean, aes(x = date, y = total_refugees, group = origin)) +
  geom_line(aes(color = origin)) +
  gghighlight(max(total_refugees) >= 5865447, use_direct_label = FALSE)
```
...
<https://github.com/curatedmess/TidyTuesday/blob/main/2023/08222023/refugees.R>

```{r}
ref_clean <- population %>% 
...
ref_clean %>% 
  ...
  geom_text(data = labels %>% mutate(group = factor(group, levels = stack_order)), aes(x = 2022.1, y = y, label = group, color = group), size = 3, hjust = 0, check_overlap = T) +
  geom_text(aes(x = year, y = year_total, label = scales::unit_format(unit = "M", scale = 1e-6)(year_total)), size = 2.5, vjust = -2.1, hjust = 0.9, check_overlap = T) +
  geom_area() +
  ...
  annotate("text", x = 2013.1, y = 6650000, label = "Total #\nof refugees\nby year", hjust = 0, vjust = 0.5, size = 2.5, color = "#000000") +
